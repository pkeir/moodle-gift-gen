...
size_t WriteCallback(void* contents, size_t size, size_t nmemb, WriteResult* result) {
        ...
        {"properties", {
            {"questions", {
                ...
                {"items", {
                    ...
                    {"required", json::array({"question", "options", "correct_answer"})}
                }}
            }}
        }},
...
std::string uploadPdfToGemini(const std::string& filepath, const std::string& api_key) {
    CURL* curl;
    CURLcode res;
    WriteResult result;
    ...
    if(res != CURLE_OK) {
        throw std::runtime_error("Failed to upload PDF: " + std::string(curl_easy_strerror(res)));
    }
...
std::string queryGemini(const std::vector<std::string>& file_ids, 
    ...
    for(const auto& file_id : file_ids) {
        content["parts"].push_back({{"file_data", {{"file_uri", "https://generativelanguage.googleapis.com/v1beta/files/" + file_id}}}});
    }
...
void deleteFileFromGemini(const std::string& file_id, const std::string& api_key) {
    CURL* curl;
    CURLcode res;
    ...
    std::string url = "https://generativelanguage.googleapis.com/v1beta/files/" + file_id + "?key=" + api_key;
...
std::vector<std::string> uploadFiles(const std::vector<std::string>& filenames, const std::string& api_key) {
...
void runQuizGeneration(const std::vector<std::string>& file_ids, const std::string& api_key) {
    int num_questions = 5;
    json schema = generateQuizSchema(num_questions);
    std::string query = "From both the text and images in these pdf files, generate " + 
                       std::to_string(num_questions) + 
                       " multiple choice questions formatted according to the provided json schema.";
    ...
    while(!satisfied) {
        ...
        satisfied = (user_input == "y" || user_input == "Y" || user_input == "yes" || user_input == "Yes");
    }
}

void cleanupFiles(const std::vector<std::string>& file_ids, const std::string& api_key) {
    for(const auto& file_id : file_ids) {
        deleteFileFromGemini(file_id, api_key);
    }
    std::cout << "All files have been successfully deleted from online storage." << std::endl;
}

int main(int argc, char* argv[]) {
    if(argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <pdf_file1> [pdf_file2] ..." << std::endl;
        return 1;
    }
